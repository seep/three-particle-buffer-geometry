import { BufferGeometry, BufferAttribute, Math as MathExt } from 'three';
 ...
 * @constructor
 */
export function ParticleBufferGeometry(opts) {
  ...
  if (typeof opts.particleGeometry === 'function') {
    ...
    generateParticleGeometry = () => opts.particleGeometry[ MathExt.randInt(0, opts.particleGeometry.length - 1) ];
  ...
  const indexBuffer = new BufferAttribute(indexTypedArray, 1);

  const pidTypedArray = new Float32Array(pidArray);
  const pidBuffer = new BufferAttribute(pidTypedArray, 1);

  const seedTypedArray = new Float32Array(seedArray);
  const seedBuffer = new BufferAttribute(seedTypedArray, 3);

  const vertexTypedArray = new Float32Array(vertexArray);
  const vertexBuffer = new BufferAttribute(vertexTypedArray, 3);
...
export function FannedCircleParticleGeometry(detail = 3) {
...
export function IcosahedronParticleGeometry() {
...
export function TetrahedronParticleGeometry() {
...
export function OctahedronParticleGeometry() {
...
export function DodecahedronParticleGeometry() {
